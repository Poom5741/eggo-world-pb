...
routerAdd("POST", "/api/v2/fix-user-wallet", (e) => {
    ...
    try {
        ...
        if (needsCreation && existingWallet) {
            ...
            userWalletsRecord.set("created", new Date().toISOString())
            userWalletsRecord.set("updated", new Date().toISOString())

            $app.save(userWalletsRecord)

            console.log("user_wallets record created:", userWalletsRecord.id)
        } else if (needsUpdate && existingWallet) {
            console.log("Updating incomplete user_wallets record for user:", user.id)
            userWalletsRecord.set("created", new Date().toISOString())
            userWalletsRecord.set("updated", new Date().toISOString())
            $app.save(userWalletsRecord)
