...
if [ -z "$PATCHES_DIR" ]; then
  if [ -d "$HOME/.config/kilo/gsd-local-patches" ]; then
    ...
    PATCHES_DIR="/Users/poom-work/tokenine/eggo-pocketbase/.qwen/gsd-local-patches"
  fi
fi
...
- **Mechanical drift** — path substitutions (e.g. `/Users/xxx/.qwen/` → `/Users/poom-work/tokenine/eggo-pocketbase/.qwen/`), variable additions (`${GSD_WS}`, `${AGENT_SKILLS_*}`), error handling additions (`|| true`)
